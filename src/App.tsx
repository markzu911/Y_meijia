...
function SmartRecTab() {
  ...
  const triggerVideoGeneration = async (imageBase64: string, styleDetails: string) => {
    ...
    try {
      ...
      const videoPrompt = `A continuous, ultra-high-quality close-up 8-second video showcasing a beautiful manicure. For the first 4 seconds, the video strictly shows the palm side of the elegant hand with the manicure (similar to the orientation of image 1). At the 4-second mark, the hand smoothly executes a natural 3D flipping rotation of the wrist to reveal the back of the hand. For the remaining 4 seconds, the hand is shown from the back with all fingers fully extended, spread out, and open wide (with fingers fully separated and open flat to show the entire elegant back of the hand perfectly, similar to a clean hand-pose reference), completely showcasing the entire back of the hand while keeping the gorgeous fingernails and custom manicure facing the camera. The core focus remains firmly on showcasing the exquisite details of the manicure. Throughout the entire video, there are subtle dynamic pulses and light rhythmic movements of the fingers to make the hand look lifelike, with absolutely no frame cuts, sudden jumps, or morphing. Consistent hand shape, skin tone, and clear studio lighting.`;
...
function CustomTab() {
  ...
  const triggerVideoGeneration = async (imageBase64: string, styleDetails: string) => {
    ...
    try {
      setVideoStep('🎨 启动 Gemini Veo 视频生成任务...');
      const videoPrompt = `A continuous, ultra-high-quality close-up 8-second video showcasing a beautiful custom manicure. For the first 4 seconds, the video strictly shows the palm side of the elegant hand with the manicure (similar to the orientation of image 1). At the 4-second mark, the hand smoothly executes a natural 3D flipping rotation of the wrist to reveal the back of the hand. For the remaining 4 seconds, the hand is shown from the back with all fingers fully extended, spread out, and open wide (with fingers fully separated and open flat to show the entire elegant back of the hand perfectly, similar to a clean hand-pose reference), completely showcasing the entire back of the hand while keeping the gorgeous fingernails and custom manicure facing the camera. The core focus remains firmly on showcasing the exquisite details of the custom manicure. Throughout the entire video, there are subtle dynamic pulses and light rhythmic movements of the fingers to make the hand look lifelike, with absolutely no frame cuts, sudden jumps, or morphing. Consistent hand shape, skin tone, and clear studio lighting.`;
